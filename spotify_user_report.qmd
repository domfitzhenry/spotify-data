...
#| fig-width: 13.5
#| fig-height: 5
#| fig-cap: "Your top 10 artists global popularity, bubble size indicates followers"
#| fig-format: svg


# Annotate the top 10 artists by current plays, and the top 5 artists by 
...
artist_annotation <- union(
  ...
  select(artist_name, current_plays, artist_popularity, followers)


top_artists %>%
  ggplot(
    aes(y = scales::rescale(current_plays, c(0, 100)), x = artist_popularity)
    ) +
  geom_jitter(
    aes(size = followers), colour = spotify_green, alpha = 0.5, 
    width = 0.5, height = 0
    ) +
  geom_text_repel(
  ...
  scale_size_binned(range = c(1,10), guide = NULL, n.breaks = 10) +
  geom_abline(
    intercept = 50, slope = 2, 
...
#| fig-width: 10
#| fig-height: 10
#| fig-cap: "An interactive sunburst chart of genres"
#| fig-format: svg
#| warning: false
