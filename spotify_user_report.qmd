...
top_artists %>%
  slice_max(order_by = current_plays, n = 10) %>%
  select(artist_name, current_plays, artist_popularity, followers) %>%
  ggplot(aes(x = current_plays, y = artist_popularity)) +
  geom_point(aes(size = followers), colour = spotify_green, alpha = 0.5) +
  geom_text_repel(aes(label = artist_name), size = 3.5, colour = txt_light_col) +
  scale_size_binned(range = c(0,10), guide = NULL) +
  labs(x = 'Your Popularity', y = "Global Popularity") +
  theme(
